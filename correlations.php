...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tgval']) && isset($_POST['tgvalb'])) {
    ...
    try {
        ...
        $command = escapeshellcmd("./correlate3.py $tgval $tgvalb $conditionString");
        $output = shell_exec($command);
        echo "<script>console.log(" . json_encode($command) . ");</script>";
        echo "<script>console.log(" . json_encode($output) . ");</script>";
        $correlationResult = $output;
    } catch (PDOException $e) {
        $correlationResult = 'Connection Failed: ' . $e->getMessage();
    }
}
?>
                ...
                <ul class="sidebar-menu">
                    ...
                    <li class="treeview">
                        ...
                        <ul class="treeview-menu">
                            ...
                            <li>
                                <a href="page-not-found.html">Page Not
                                    Found</a>
                            </li>
                            <li>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-xl-6 col-12">
                            ...
                            <body>
                            <form action="correlations.php" method="post">
                                <div class="radio-group-container">
                                    <div class="radio-group">
                                        <?php foreach ($dbfs as $index => $value): ?>
                                            <label><?php echo htmlspecialchars($nms[$index]); ?>
                                                <input type="radio" name="tgval"
                                                       value="<?php echo htmlspecialchars($value); ?>" <?php echo $index === 0 ? 'checked' : ''; ?>>
                                    ...
                                    <div class="radio-group">
                                        <?php foreach ($dbfs as $index => $value): ?>
                                            <label><?php echo htmlspecialchars($nms[$index]); ?>
                                                <input type="radio" name="tgvalb"
                                                       value="<?php echo htmlspecialchars($value); ?>" <?php echo $index === 1 ? 'checked' : ''; ?>>
                                                <span class="checkmark">✔</span>
                                            </label>
                                        <?php endforeach; ?>
                                    </div>
                                ...
                                <input type="submit" value="Calculate Pearson"/>
                            </form>
                            <p class="stat-header">Correlation Result:</p>
                            <?php if (!empty($correlationResult)): ?>
                                <div style="margin-top: 20px;">
                                    <pre class="stat-result"><?php echo htmlspecialchars($correlationResult); ?></pre>
                                </div>
                            <?php endif; ?>
                                ...
                                document.querySelectorAll(".radio-group").forEach(group => {
                                    group.querySelectorAll("label").forEach(label => {
                                        label.addEventListener("click", function () {
                                            group.querySelectorAll("label").forEach(otherLabel => {
                                                otherLabel.classList.remove("selected");
                                                otherLabel.removeAttribute("data-checked");
                                            });
